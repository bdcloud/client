...
# coding=utf-8
# crontab -e
# * * * * * /home/xxx/anaconda2/bin/python /home/xxx/client/StatusInfo.py
...
def post_data(url, data):
    try:
	data1 = [{'headers': {'a': 'a1', 'b': 'b1'}, 'body':data}]
        r = requests.post(url, json.dumps(data1))
        if r.text:
            logging.info(r.text)
        else:
...
def agg_sys_info():
    ...
    logging.info('----------------------------------------------------------')
    result = post_data("http://{0}/monitor/received/status/info/".format(server_ip), json_data)
    return True
...
def get_pid():
    ...
    pid = str(os.getpid())
    with open(BASE_DIR+"/statusinfod.pid", "wb+") as pid_file:
